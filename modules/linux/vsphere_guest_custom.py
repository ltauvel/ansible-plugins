...
# BEGIN : ADDED BY LTAUVEL
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
import collections
import random
import re
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
# END : ADDED BY LTAUVEL
...
options:
  ...
  esxi:
    description:
      - Dictionary which includes datacenter, hostname on which the VM should be created. For standalone ESXi hosts, ha-datacenter should be used as the datacenter name. For cluster host_smart_select and host_smart_select_vm_regex to try to split multiple instances of a same service on different ESX hosts.
    required: false
    default: null
  state:
    description:
...
# Deploy a guest from a template on aspecific ESX host
- vsphere_guest:
    vcenter_hostname: vcenter.mydomain.local
    username: myuser
    password: mypass
...
# Deploy a guest from a template on a ESX host choose by trying to split multiple services vm on different ESX hosts
- vsphere_guest:
    vcenter_hostname: vcenter.mydomain.local
    username: myuser
    password: mypass
    ...
    vm_extra_config:
      folder: MyFolder
      datastore: mydatastorenameregex
    esxi:
      datacenter: mydatacenter
      host_smart_select: yes
      host_smart_select_vm_regex: myvmnameregex
...
# BEGIN : ADDED BY LTAUVEL
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
def find_best_host(vsphere_client, cluster_name, re_vm_name, module):
    # Find a the most appropriate host to create a vm on
    # according to the specified vm name regular expression

    
    # Find the specified cluster
    cluster_obj = [k 
        for k, v in vsphere_client.get_clusters().items() 
            if v == cluster_name
    ][0] if cluster_name else None

    # Looking for the available hosts in the cluster
    # where available means green status
    available_hosts_obj = [k 
        for k, v in vsphere_client.get_hosts(cluster_obj).items() 
            if vsphere_client._get_object_properties(k, property_names=['overallStatus']).get_element_propSet()[0].Val == 'green'
    ]

    # Get a list of host hosting vm corresponding
    # to the re_vm_name specified
    vm_hosts_obj = [vsphere_client._get_object_properties(k, property_names=['runtime.host']).get_element_propSet()[0].Val 
        for k, v in vsphere_client._get_managed_objects(MORTypes.VirtualMachine, from_mor=cluster_obj).iteritems()
            if re.match(re_vm_name, v)
    ]

    # Generating a list of available target hosts ordered by priority descending.
    # Priority is determined by the number of matching virtual machine on each host.
    # The greater means the less priority. 
    ordered_hosts_obj = [i
        for i in collections.Counter(vm_hosts_obj).most_common()
            if i[0] in available_hosts_obj
    ]
    ordered_hosts_obj += [(h, 0)
        for h in available_hosts_obj
            if h not in [i[0] for i in ordered_hosts_obj]
    ]
    
    # Randomly selecting a host having the highest priority.
    return random.sample(
        [h
            for h in ordered_hosts_obj
                # selecting the all the hosts having the maximum priority
                if h[1] == ordered_hosts_obj[-1][1]
        ], 1
    )[0][0]
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
# END : ADDED BY LTAUVEL
...
def deploy_template(vsphere_client, guest, resource_pool, template_src, esxi, module, cluster_name, snapshot_to_clone, power_on_after_clone, vm_extra_config):
    ...
    if esxi:
        ...
        esxi_hostname = esxi['hostname'] if 'hostname' in esxi.keys() else None
        
        # BEGIN : ADDED BY LTAUVEL
        # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
        esxi_host_smart_select = esxi['host_smart_select'] if 'host_smart_select' in esxi.keys() else False
        esxi_host_smart_select_vm_regex = esxi['host_smart_select_vm_regex'] if 'host_smart_select_vm_regex' in esxi.keys() else re.sub(r'^(.+)[0-9]+$', r'\1', guest)
        # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
        # END : ADDED BY LTAUVEL
        ...
        # BEGIN : ADDED BY LTAUVEL
        # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
        # Grab the host managed object reference of the select esxi
        if esxi_host_smart_select:
            if cluster_name:
                hostmor = find_best_host(vsphere_client, cluster_name, esxi_host_smart_select_vm_regex, module)
            else:
                module.fail_json(msg="Missing 'cluster' parameter to find the appropriate host")
        ...
        # Grab the host managed object reference of the esxi_hostname
            ...
            if esxi_hostname:
                ...
                module.fail_json(msg="Missing 'esxi:[hostname]' parameter")
            # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
            # END : MODIFIED BY LTAUVEL
        ...
        module.fail_json(msg="You need to specify either esxi:[datacenter,hostname] or [cluster, esxi:[datacenter,smart_host_select]] or [cluster,resource_pool]")
    ...
    try:
        if not vmTarget:
            ...
            # BEGIN : ADDED BY LTAUVEL
            # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
            if vm_extra_config.get("datastore") is not None and vm_extra_config.get("datastore") != "":
                # if a Datastore is specified, add it to the clone spec                
                mor = None
                for ds in VIProperty(vsphere_client, hostmor).datastore:
                    if re.match(vm_extra_config.get("datastore"), ds.info.name):
                        mor = ds._obj; break
                cloneArgs["datastore"] = mor
            # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
            # END : ADDED BY LTAUVEL
...
def main():
    ...
    proto_esxi = {
        ...
        'hostname': basestring,
        'smart_host_select': bool,
        'host_smart_select_vm_regex': basestring,
    }
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    # END : MODIFIED BY LTAUVEL
